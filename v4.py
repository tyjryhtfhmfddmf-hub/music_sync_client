...
Label(queue_frame, text="🎵 Up Next").pack()

queue_list = Listbox(queue_frame, width=50, selectmode=SINGLE)
queue_list.pack()

btn_frame = Frame(queue_frame)
btn_frame.pack()

Button(btn_frame, text="Move Up", command=lambda: move_song(-1)).pack(side=LEFT, padx=5)
Button(btn_frame, text="Move Down", command=lambda: move_song(1)).pack(side=LEFT, padx=5)
...
def refresh_queue_view():
    """Update the visible queue list."""
    queue_list.delete(0, END)
    if not playlist:
        return
    # Show all songs, highlight the current one
...
def move_song(direction):
    ...
    if 0 <= new_index < len(playlist):
        ...
        refresh_queue_view()
        queue_list.select_set(new_index)


queue_list.bind("<Double-Button-1>", lambda e: play_selected_song())

def play_selected_song():
    sel = queue_list.curselection()
    if not sel: return
    index = sel[0]
    load_song(playlist[index])
