...
def sync_current_playlist():
    ...
    if not playlist:
        update_status("No playlist to sync")
        return
...
def compare_libraries():
    ...
    # Send request for library info
    send_command("request_library")
    update_status("Requesting library comparison...")
...
def compare_playlists(received_playlist):
    ...
    if missing_songs:
        ...
        messagebox.showwarning(
            "Missing Songs",
            f"You are missing {len(missing_songs)} songs from this playlist:\n\n{missing_text}"
        )
...
def show_library_comparison(data):
    ...
    if only_local:
        ...
        sample = list(only_local)[:5]
        msg += "  • " + "\n  • ".join(sample)
        if len(only_local) > 5:
            msg += f"\n  ... and {len(only_local) - 5} more\n"
        msg += "\n"
    
    if only_remote:
        msg += f"Songs only they have: {len(only_remote)}\n"
        sample = list(only_remote)[:5]
        msg += "  • " + "\n  • ".join(sample)
        if len(only_remote) > 5:
            msg += f"\n  ... and {len(only_remote) - 5} more"
    ...
    text_widget = Text(comparison_window, wrap=WORD, padx=10, pady=10)
...
def process_command(cmd_data):
    ...
    elif command == "sync_playlist" and data is not None:
...
session_frame.grid(row=0, column=0, columnspan=3, pady=5)
...
sync_frame.grid(row=1, column=0, columnspan=3, pady=5)
